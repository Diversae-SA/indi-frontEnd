...
const columns = [
  ...
  { data: 'name', title: 'Tipo de Dato' },
]
const idData = ref()
const emit = defineEmits(['updateTable'])
const buttonTable = [
  { button: 'edit', permission: 'type_data edit' },
  { button: 'delete', permission: 'type_data delete' },
]
...
async function onStore() {
  if (!isLoading.value) {
    ...
    try {
      ...
      const url = updateData.value ? '/type_data/' + idData.value : '/type_data'
...
async function handleEdit(id: number) {
  idData.value = id
  await $fetch('type_data/' + id).then((result: Department) => {
...
async function DeletedTraining() {
  try {
    updateTableEvent.value = false
    emit('updateTable')
    await $fetch(`/type_data/${idData.value}`, { method: 'DELETE' })
...
<template>
  ...
  <div
    v-if="hasPermission('type_data create')"
  ...
  <VModal
    is="form"
    :open="modalNew"
    :title="updateData ? 'Actualizar Tipo de Dato' : 'Registrar nuevo Tipo de Dato'"
    size="small"
    actions="right"
    :noclose="true"
  ...
  >
    <template #content>
      <div class="modal-form">
        <VField
          id="name"
          label="Nombre para el Tipo de Dato"
        >
