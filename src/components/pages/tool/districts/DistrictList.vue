...
const columns = [
  { data: 'id', title: 'ID' },
  { data: 'name', title: 'Nombre', typeSearch: 'input' },
  { data: 'code', title: 'Codigo', typeSearch: 'input' },
  { data: 'department.name', title: 'Departamento', typeSearch: 'input' },
]
const idData = ref(null)
const emit = defineEmits(['updateTable'])
const buttonTable = [
  { button: 'edit', permission: 'districts edit' },
  { button: 'delete', permission: 'districts delete' },
]

const handleEdit = (id: number) => {
  router.push({ path: '/tool/districts/update/' + id })
}
...
async function DeletedItem() {
  try {
    ...
    emit('updateTable')
    const res = await api.delete(`/districts/${idData.value}`)
