...
export function transformExampleMarkup(raw) {
    let output = raw;
    let content = null;
...
export function transformSlots(source, condition = '') {
    if (source.includes('<!--code-->') && source.includes('<!--example-->')) {
        return `<template ${condition} #default>${source}`
            .replace(`<!--code-->`, `</template><template ${condition} #code>\n<slot name="code"><div v-pre>`)
            .replace(`<!--/code-->`, `</div></slot>\n</template>`)
            .replace(`<!--example-->`, `<template ${condition} #example>\n<slot name="example">`)
    ...
    if (source.includes('<!--example-->')) {
        return `<template ${condition} #default>${source}`
            .replace(`<!--example-->`, `</template><template ${condition} #example>\n<slot name="example">`)
