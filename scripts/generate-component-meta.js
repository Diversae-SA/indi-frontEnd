...
async function main() {
    ...
    const content = [
        ...
        ``,
        `export const components = ${JSON.stringify(Object.keys(components))} as const`,
        ``,
        ...Object.entries(components).map(([name, meta]) => {
            return `export const ${name}Meta = ${JSON.stringify(meta)} satisfies ComponentMeta`;
        }),
    ].join('\n');
    await writeFile(out, content);
}
async function walk(components, checker, dir) {
    const files = await readdir(dir);
    for (const file of files) {
        const path = join(dir, file);
        const stat = await lstat(path);
        if (stat.isDirectory()) {
            await walk(components, checker, path);
        }
...
async function extractMeta(checker, path) {
    ...
    const meta = {
        ...
        exposed: metaFields.exposed.filter((field) => {
            const isProps = metaFields.props?.findIndex(prop => prop.name === field.name) >= 0;
            const isEvent = metaFields.events?.findIndex((event) => `on${event.name}`.toLowerCase() === field.name?.toLowerCase()) >= 0;
...
//# sourceMappingURL=generate-component-meta.js.map